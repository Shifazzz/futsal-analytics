...
with col1:
    st.metric(
        ...
        delta=f"Avg ₹{total_revenue/len(df_filtered):,.0f}/mo"
    )
...
with col3:
    st.metric(
        ...
        delta=f"Avg ₹{total_profit/len(df_filtered):,.0f}/mo"
    )
...
with col5:
    st.metric(
        label="Best Month",
        value=best_month,
        delta=f"₹{df_filtered['Total_Income'].max():,.0f}"
    )
...
with tab2:
    ...
    with col1:
        st.metric("Sports Revenue", f"₹{sports_revenue:,.0f}", 
                 delta=f"{(sports_revenue/total_revenue*100):.1f}% of total")
    with col2:
        st.metric("Sports Expenses", f"₹{sports_expenses:,.0f}",
                 delta=f"{(sports_expenses/sports_revenue*100):.1f}% of revenue")
    with col3:
        st.metric("Sports Profit", f"₹{sports_profit:,.0f}",
    ...
    fig = go.Figure(data=[go.Bar(
        ...
        marker_color='#3498db',
        text=[f"₹{v:,.0f}" for v in expense_categories.values()],
        textposition='auto'
    )])
...
with tab4:
    ...
    revenue_summary.columns = ['Month', 'Sports (₹)', 'F&B (₹)', 'Total (₹)', 'Sports %', 'F&B %']
    
    # Format numbers
    for col in ['Sports (₹)', 'F&B (₹)', 'Total (₹)']:
        revenue_summary[col] = revenue_summary[col].apply(lambda x: f"₹{x:,.0f}")
...
with tab5:
    ...
    fig = go.Figure(data=[go.Bar(
        ...
        orientation='h',
        marker_color='#e67e22',
        text=[f"₹{v:,.0f}" for v in all_expenses.values()],
        textposition='auto'
    )])
...
with tab6:
    ...
    with col1:
        ...
        fig = go.Figure(go.Waterfall(
            ...
            text=[f"₹{total_revenue:,.0f}", f"-₹{total_expenses:,.0f}", f"₹{total_profit:,.0f}"],
            textposition="auto",
    ...
    with col3:
        ...
        fig = go.Figure(data=[
            go.Bar(
                ...
                text=[
                    f"₹{df_filtered.loc[best_month_idx, 'Total_Surplus']:,.0f}",
                    f"₹{df_filtered.loc[worst_month_idx, 'Total_Surplus']:,.0f}",
                    f"₹{df_filtered['Total_Surplus'].mean():,.0f}"
                ],
                textposition='auto'
            )
        ])
    ...
    for col in ['Sports Profit', 'F&B Profit', 'Total Profit']:
        profitability_data[col] = profitability_data[col].apply(lambda x: f"₹{x:,.0f}")
...
with tab7:
    ...
    insights.append({
        "title": "💰 Revenue Mix",
        "desc": f"Sports contributes **{sports_pct:.1f}%** (₹{df_filtered['Sports_Income_Total'].sum():,.0f}) and F&B contributes **{fb_pct:.1f}%** (₹{df_filtered['FB_Income_Total'].sum():,.0f}) of total revenue.",
        "priority": "Info"
    })
    ...
    insights.append({
        ...
        "desc": f"Generated **₹{best_month_revenue:,.0f}** in revenue. Analyze what made this month successful.",
        "priority": "Info"
    })
